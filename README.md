...
<p align="center">
 ...
 после чего по самым большим весам кодера идем до его входного слоя, на котором, через веса модели, ищутся самые важные фичи для классификации уже в исходном пространстве, по формуле:
 
$$
\omega_{j k}- mean\left(\omega_{j d}\right)>\beta * std\left(\omega_{j d}\right)
$$

Здесь $\omega_{j d}$ - веса модели на входном слое, т. е. согласно этой формуле k-ая фича - является важной. $\beta$ - гиперпараметр, который подбирался так, чтобы мы находили ~ 150 важных фичей. Номера этих фичей лежат в reports/mz_features_40%_group.txt
### Cross noise
